...
1. Pick the range and record its base SHA. Whole branch: the SHA from `git merge-base origin/main HEAD`. One task: the commit recorded before it started — never `HEAD~1`, which drops all but the last commit.
