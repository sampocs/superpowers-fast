...
assert_equal(marketplace.get("name"), "superpowers-fast-dev", "marketplace name")
assert_equal(
    marketplace.get("interface", {}).get("displayName"),
    "Superpowers Fast Dev",
...
matching_plugins = [plugin for plugin in plugins if plugin.get("name") == "superpowers-fast"]
assert_equal(len(matching_plugins), 1, "superpowers-fast plugin entry count")
